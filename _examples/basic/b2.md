...
The _chamber_ volumes are associated with the `flux` digitization (one of the available GEMC pre-built routines)
...
```shell
python3 -m analyzer CO2_t0_digitized.csv totEdep --kind csv
```
...
```shell
python3 -m analyzer CO2_t0_true_info.csv E --kind csv --data true_info
```
