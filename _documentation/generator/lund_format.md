...
GEMC reads event particle definitions from LUND files with:
...
| Quantity                        | Unit                                                       |
|---------------------------------|------------------------------------------------------------|
| Momentum (columns 7–9)          | GeV                                                        |
| Vertex position (columns 12–14) | cm                                                         |
| Angles                          | Reconstructed from the momentum vector — not read directly |
...
1. one header line;
2. `N` particle lines, where `N` is the first value on the header line.

In single-threaded mode, LUND event 0 is used for GEMC event 0, LUND event 1 for GEMC event 1, and so on.
...
The header must contain at least 10 numeric values. Up to 100 values are allowed.
Only **column 1** (the particle count) is used by GEMC to create primaries.
The remaining columns follow the LUND convention and are preserved in the output but
not used to generate particles.
...
| 5 | First particle z component of spin. User-defined. |
| 6 | Beam type (e.g. electron = 11, photon = 22). User-defined. |
| 7 | Beam energy in GeV. User-defined. |
| 8 | Interacted nucleon ID (e.g. 2212 or 2112). User-defined. |
| 9 | Process ID. User-defined. |
| 10 | Event weight. User-defined. |
| 11–100 | Optional numeric values. |

Column 1 must be a non-negative integer.
...
After the header (and optional blank separator), the event must contain exactly `N` particle lines.
Each line must contain at least 14 numeric columns. Up to 100 columns are allowed.
Columns shown in **bold** are used by GEMC to create primaries.

{:.zebra .compact-table}

| Column | Quantity |
| --- | --- |
| **1** | **Particle index. Must start at 1 and increment by 1 for each particle.** |
| 2 | Lifetime in ns. User-defined. |
| **3** | **Type. Only type `1` is propagated to Geant4 and appears in the generated output.** |
| **4** | **PDG particle ID.** |
...
Columns 1, 3, 4, 5, and 6 must be integer-valued.

> [!NOTE]
> GEMC creates a Geant4 primary only for particle lines where **type = 1**. Lines with any
> other type value are read and stored but not passed to the Geant4 particle gun.
...
- one electron (PDG 11) with momentum `(0.0, 0.0, 2.3)` GeV at `(0.0, 0.0, 0.0)` cm;
- one proton (PDG 2212) with momentum `(0.0, 0.1, 1.2)` GeV at `(0.0, 0.0, 0.0)` cm.

Consecutive events are separated by a blank line, or may follow directly one after another.

<br/>

## Multi-Event Example

The following shows the first two deep inelastic scattering events. A blank line separates
the two records. Particles with type ≠ 1 (e.g. type 21, 13, 12, 11) are stored in the
output but not propagated.
...
LUND event 0 (22 particles) is used for GEMC event 0; LUND event 1 (18 particles) is used for
GEMC event 1. Only particles with type = 1 in each event are passed to the Geant4 particle gun.
