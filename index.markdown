...
<small>This site refers to the latest **GEMC** project (version 3 and above).
For **CLAS12 simulations** refer to [this page](https://github.com/gemc/clas12Tags).</small>
...
- Import geometry from `ASCII`, `SQLite`, `GDML`, and `CAD mesh` files
- Built-in sensitive detectors: `dosimeter`, `flux`, `particle_counter`
- `ASCII`, `CSV`, `JSON`, [`ROOT`](https://root.cern) output formats
- Built-in Python `analyzer` for reading and plotting simulation output
...
The total energy deposited is plotted using the analyzer module."
%}
